...
print('\n\27[1;36m￤تم آدخآل آيدي آلمـطـور بنجآح , سـوف يتم تشـغيل سورس ايجل الآن .\n￤Success Save ID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
js = Token:match("(%d+)")
redis:mset(
js..":VERSION","1.0",
...
function Start_Bot() 
local TokenBot = io.open('./MaSa/Token.txt', "r")
...
if not SUDO_ID then io.popen("rm -fr ./MaSa/Token.txt") end
SUDO_USER = redis:hgetall(js..'username:'..SUDO_ID).username
version = redis:get(js..":VERSION")
DataCenter = redis:get(js..":DataCenter:")

local ok, ERROR =  pcall(function() loadfile("./MaSa/functions.lua")() end)
if not ok then 
print('\27[31m! Error File Not "Run MaSa/functions.lua" !\n\27[39m')
print(tostring(io.popen("lua MaSa/functions.lua"):read('*all')))
end

local ok, ERROR =  pcall(function() loadfile("./MaSa/locks.lua")() end)
if not ok then 
print('\27[31m! Error File Not "Run MaSa/locks.lua" !\n\27[39m')
print(tostring(io.popen("lua MaSa/locks.lua"):read('*all')))
end
...
..'¦ Run_Scrpt: \27[1;34m./MaSa/Script.lua\027[0;32m \n'
...
local ok, i =  pcall(function() ScriptFile = loadfile("./MaSa/Script.lua")() end)
if not ok then  
print('\27[31m! Error File Not "Run MaSa/Script.lua" !\n\27[39m')
print(tostring(io.popen("lua MaSa/Script.lua"):read('*all')))
end
...
UpdateSource() dofile("./MaSa/Run.lua")
